...
class CustomOutputParser(AgentOutputParser):
    @staticmethod
    def parse(text: BaseMessage) -> Union[AgentAction, AgentFinish]:
        """
        解析代理的输出。
        :param text: LLM 生成的原始文本。
        :return: 解析后的 AgentAction 或 AgentFinish 对象。
        """
        if not isinstance(text, AIMessage):
            raise TypeError(f"Expected an AI message got {type(text)}")
        # 假设输出包含特定的标记来区分动作和完成状态
        print(text)
        # function_name = text.tool_calls[0]["name"]
        function_call = text.additional_kwargs.get("tool_calls", [{}])
        function_call = function_call[0].get("function",{})
        if function_call:
            return AgentAction(tool = function_call['name'], tool_input = function_call['arguments'])
        else:
            raise ValueError(f"没有调用工具")



# chain = llm_tools | CustomOutputParser()
res = llm_tools.invoke("查询北京的天气，城市代码为CH010100")
outputparser = CustomOutputParser()
res = outputparser.parse(res)
print(res)